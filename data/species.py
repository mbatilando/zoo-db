...
def parse_species_page(url):
  ...
  has_genus_and_species = ("genus" in panel_info) and ("species" in panel_info)
  if (("'" not in common_name) and has_genus_and_species
      and panel_info["genus"].isalpha() and panel_info["species"].isalpha()):
    species_info["common_name"] = common_name
    species_info["scientific_name"] = ("%s %s" %
        (panel_info["genus"], panel_info["species"]))
